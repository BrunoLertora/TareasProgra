...
class Usuario:
    def __init__(self,usuario_id,usuario,contraseña,reproducciones):
        self.usuario_id = usuario_id
...
def cargar_datos():
    ...
    with open('tracks.csv', encoding='utf8') as file:
        ...
        for fila in datos:
            try:
                tracks.append(Track(fila['Artist'],fila['Album'],fila['Track'],fila['Duration'],generos[fila['Artist']]))
            except:
    ...
    with open('reproduccion.csv', encoding='utf8') as file:
        ...
        for fila in datos:
            ...
            if fila['usuario_id'] not in reproducciones:
                reproducciones[fila['usuario_id']] = [Reproduccion(fila['id_reproduccion'],fila['usuario_id'],cancion,fila['fecha'])]
            else:
                reproducciones[fila['usuario_id']].append(Reproduccion(fila['id_reproduccion'],fila['usuario_id'],cancion,fila['fecha']))
    usuarios = {}
    with open('usuarios.csv', encoding='utf8') as file:
        datos = csv.DictReader(file)
        for fila in datos:
            usuarios[fila['usuario_id']] = Usuario(fila['usuario_id'], fila['usuario'],fila['contraseña'],reproducciones[fila['usuario_id']])
